  ...
  try {
    ...
    async (applicant) => {
      ...
      if (file_id) {
        ...
        // Use OS temp directory and create it if it doesn't exist
        const os = await import('os');
        const path = await import('path');
        const fs = await import('fs');
        
        const tempDir = path.join(os.tmpdir(), 'unmasker_cv_processing');
        if (!fs.existsSync(tempDir)) {
          fs.mkdirSync(tempDir, { recursive: true });
        }
        
        const tempFilePath = path.join(tempDir, `cv_${applicant_id}_${Date.now()}.pdf`);
